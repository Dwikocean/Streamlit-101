...
if menu == "Dashboard":
    ...
    if not df.empty:
        product_id = st.selectbox("Select Product", df["Product ID"].unique())
        selected_product = df[df["Product ID"] == product_id].iloc[0]
        ...
        if st.button("Update"):
            df.loc[df["Product ID"] == product_id, "Stock"] = new_stock
            df.loc[df["Product ID"] == product_id, "Price"] = new_price
    ...
    discounted_products = apply_discount(df)
